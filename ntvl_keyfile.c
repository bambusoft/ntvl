 ...
 */
ssize_t ntvl_parse_hex( uint8_t * keyBuf, 
                       size_t keyLen, 
                       const char * textKey,
                       size_t textLen) {
    ssize_t retval=0;
...
static int parseKeyLine( ntvl_cipherspec_t * spec, 
                         const char * linein ) {
    /* parameters are separated by whitespace */
    char line[NTVL_KEYFILE_LINESIZE];
    char * lp=line;
    const char * token;
    strncpy( line, linein, NTVL_KEYFILE_LINESIZE );

    memset( spec, 0, sizeof( ntvl_cipherspec_t ) );
    ...
    token = strsep( &lp, DELIMITERS );
    if ( !token ) { goto error; }
    strncpy( (char *)spec->opaque, token, NTVL_MAX_KEYSIZE );
...
int validCipherSpec( const ntvl_cipherspec_t * k,
                     time_t now ) {
 ...
 */
int ntvl_read_keyfile( ntvl_cipherspec_t * specs,     /* fill out this array of cipherspecs */
                      ...
                      const char * ctrlfile_path )  /* path to control file */
{
    ...
    if ( fp ) {
        ...
        while ( idx < numspecs ) {
            ntvl_cipherspec_t * k = &(specs[idx]);
            fgets( line, NTVL_KEYFILE_LINESIZE, fp );
            ++lineNum;
            ...
            if ( strlen(line) > 1 ) {
                ...
                } else traceEvent( TRACE_WARNING, "Failed to decode line %u\n", lineNum );
            }
