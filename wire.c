...
/** Routines for encoding and decoding ntvl packets on the wire.
 *
...
int encode_common( uint8_t * base, 
    ...
    flags  = common->pc & NTVL_FLAGS_TYPE_MASK;
    flags |= common->flags & NTVL_FLAGS_BITS_MASK;

    encode_uint16( base, idx, flags );
    encode_buf( base, idx, common->community, NTVL_COMMUNITY_SIZE );
...
int decode_common( ntvl_common_t * out,
    ...
    out->pc = ( out->flags & NTVL_FLAGS_TYPE_MASK );
    out->flags &= NTVL_FLAGS_BITS_MASK;

    decode_buf( out->community, NTVL_COMMUNITY_SIZE, base, rem, idx );
...
int decode_REGISTER( ntvl_REGISTER_t * reg,
...
int decode_REGISTER_SUPER( ntvl_REGISTER_SUPER_t * reg,
                           const ntvl_common_t * cmn, /* info on how to interpret it */
                           const uint8_t * base,
                           size_t * rem,
                           size_t * idx ) {
...
int decode_REGISTER_ACK( ntvl_REGISTER_ACK_t * reg,
                         const ntvl_common_t * cmn, /* info on how to interpret it */
                         const uint8_t * base,
                         size_t * rem,
                         size_t * idx ) {
...
int decode_REGISTER_SUPER_ACK( ntvl_REGISTER_SUPER_ACK_t * reg,
                               const ntvl_common_t * cmn, /* info on how to interpret it */
                               const uint8_t * base,
                               size_t * rem,
                               size_t * idx ) {
...
int decode_PACKET( ntvl_PACKET_t * pkt,
                   const ntvl_common_t * cmn, /* info on how to interpret it */
                   const uint8_t * base,
                   size_t * rem,
                   size_t * idx ) {
