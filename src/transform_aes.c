...
#define NTVL_AES_NUM_SA                  32 /* space for SAa */

#define NTVL_AES_TRANSFORM_VERSION       1  /* version of the transform encoding */
#define NTVL_AES_IVEC_SIZE               32 /* Enough space for biggest AES ivec */
...
struct sa_aes {
    ...
    ntvl_aes_ivec_t      enc_ivec;       /* tx CBC state */
    AES_KEY             dec_key;        /* tx key */
    ntvl_aes_ivec_t      dec_ivec;       /* tx CBC state */
};
...
static int transop_encode_aes( ntvl_trans_op_t * arg,
    ...
    if ( (in_len + TRANSOP_AES_NONCE_SIZE) <= NTVL_PKT_BUF_SIZE ) {
        if ( (in_len + TRANSOP_AES_NONCE_SIZE + TRANSOP_AES_SA_SIZE + TRANSOP_AES_VER_SIZE) <= out_len ) {
            ...
            encode_uint8( outbuf, &idx, NTVL_AES_TRANSFORM_VERSION );
            ...
            *pnonce = rand();
            ...
            memset( &(sa->enc_ivec), 0, sizeof(NTVL_AES_IVEC_SIZE) );
    ...
    if ( priv->num_sa < NTVL_AES_NUM_SA ) {
        ...
        if ( sep ) {
            ...
            if ( pstat > 0 ) {
                ...
                /* Use NTVL_MAX_KEYSIZE because the AES key needs to be of fixed
...
int transop_aes_init( ntvl_trans_op_t * ttt ) {
    ...
    if ( NULL != priv ) {
        ...
        ttt->transform_id = NTVL_TRANSFORM_ID_AESCBC;
...
#endif /* #if defined(NTVL_HAVE_AES) */
