...
void traceEvent(int eventTraceLevel, char* file, int line, char * format, ...) {
  ...
  if(eventTraceLevel <= traceLevel) {
    char buf[2048];
    char out_buf[640];
    char theDate[NTVL_TRACE_DATESIZE];
    char *extra_msg = "";
    ...
    strftime(theDate, NTVL_TRACE_DATESIZE, "%d/%b/%Y %H:%M:%S", localtime(&theTime));
    ...
    if(useSyslog) {
      if(!syslog_opened) {
        openlog("ntvl", LOG_PID, LOG_DAEMON);
        syslog_opened = 1;
      }
...
extern char * sock_to_cstr( ntvl_sock_str_t out, const ntvl_sock_t * sock ) {
    int r;
